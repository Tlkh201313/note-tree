...
push('<defs>');
push('<linearGradient id="sky" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#0b1220"/><stop offset="1" stop-color="#111a2b"/></linearGradient>');
...
push(`<text x="34" y="52" font-family="${SANS}" font-size="27" font-weight="600" fill="#e2e8f0">🌳 note-tree</text>`);
push(`<text x="34" y="78" font-family="${SANS}" font-size="15" fill="#94a3b8">Memory that grows, not memory that bloats.</text>`);

/* ----------------------------------------------------------- tree panel -- */

const PANEL = { x: 26, y: 104, w: 372, h: 560 };
push(`<rect x="${PANEL.x}" y="${PANEL.y}" width="${PANEL.w}" height="${PANEL.h}" rx="12" fill="#0d1524" stroke="#1e293b"/>`);
...
push('</svg>');
push(`<text x="${PANEL.x + 14}" y="${PANEL.y + 22}" font-family="${MONO}" font-size="12" fill="#64748b">one leaf per note · one branch per session</text>`);
push(`<text x="${PANEL.x + 14}" y="${PANEL.y + PANEL.h - 14}" font-family="${MONO}" font-size="12" fill="#94a3b8">${L.counts.live} notes · ${L.counts.sessions} sessions · ${L.stage}</text>`);

/* ----------------------------------------------------------- seed panel -- */

const SEED = { x: 420, y: 104, w: 794, h: 560 };
push(`<rect x="${SEED.x}" y="${SEED.y}" width="${SEED.w}" height="${SEED.h}" rx="12" fill="#0d1524" stroke="#1e293b"/>`);
push(`<text x="${SEED.x + 16}" y="${SEED.y + 24}" font-family="${MONO}" font-size="12" fill="#64748b">what the next session actually receives — every session, in every agent</text>`);
...
for (const raw of seed.text.split('\n')) {
  if (y > SEED.y + SEED.h - 46) {
    line(SEED.x + 16, '…', '#475569');
    break;
  }
  const note = raw.match(/^(\w+) (\w+) (\S+) (.*)$/);
  if (raw.startsWith('<note-tree-memory') || raw.startsWith('</note-tree-memory')) {
    line(SEED.x + 16, raw, '#38bdf8');
  } else if (raw.startsWith('## ')) {
    y += 6;
    line(SEED.x + 16, clip(raw), '#e2e8f0', ' font-weight="600"');
  } else if (note) {
    const [, id, kind, age, rest] = note;
    const style = kindStyle(kind);
    const head = `${id} ${kind} ${age} `;
    line(SEED.x + 16, id, '#475569');
    push(`<text x="${n2(SEED.x + 16 + (id.length + 1) * CHAR)}" y="${y}" font-family="${MONO}" font-size="11" fill="${style.hex}">${esc(kind)}</text>`);
    push(`<text x="${n2(SEED.x + 16 + (id.length + kind.length + 2) * CHAR)}" y="${y}" font-family="${MONO}" font-size="11" fill="#475569">${esc(age)}</text>`);
    push(`<text x="${n2(SEED.x + 16 + head.length * CHAR)}" y="${y}" font-family="${MONO}" font-size="11" fill="#cbd5e1">${esc(clip(rest).slice(0, MAX_CHARS - head.length))}</text>`);
  } else {
    line(SEED.x + 16, clip(raw), '#64748b');
  }
  y += 17;
}

push(
  `<text x="${SEED.x + 16}" y="${SEED.y + SEED.h - 16}" font-family="${MONO}" font-size="12" fill="#94a3b8">` +
...
for (const k of KIND_LEGEND.filter((k) => seen.has(k.kind))) {
  ...
  push(`<text x="${n2(lx + 12)}" y="${H - 14}" font-family="${MONO}" font-size="11" fill="#64748b">${k.label}</text>`);
  lx += 26 + k.label.length * 6.6;
}
push(`<text x="34" y="${H - 14}" font-family="${MONO}" font-size="11" fill="#64748b">0 dependencies</text>`);
